import os, argparse, numpy as np, librosa, soundfile as sf
...
def melspec(wav_path, sr=16000, n_mels=128, win_ms=25, hop_ms=10):
    ...
    S = librosa.feature.melspectrogram(
        ...
        n_fft=int(sr*win_ms/1000), hop_length=int(sr*hop_ms/1000), power=2.0
    )
...
def main():
    ...
    for i,f in enumerate(sorted(wavs), 1):
        ...
        if os.path.exists(out_path):
            print(f"[{i}/{len(wavs)}] skip {clip_id} (exists)");  continue
        try:
            M = melspec(in_path)
            np.save(out_path, M)
            print(f"[{i}/{len(wavs)}] wrote {out_path} {M.shape}")
        except Exception as e:
            print(f"[{i}/{len(wavs)}] ERROR {clip_id}: {e}")
