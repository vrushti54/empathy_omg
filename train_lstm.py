...
def main():
    ...
    model = LSTMRegressor(n_mels=n_mels, hidden=128, layers=1, bidir=True).to(device)
    ...
    with open("results/lstm_audio_melspec.json","w") as f: json.dump(best[1], f, indent=2)
    print("Saved results/lstm_audio_melspec.json")
